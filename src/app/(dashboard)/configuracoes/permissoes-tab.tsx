...
export function PermissoesTab({
  ...
  const editing = cargos.find((c) => c.id === editingId) ?? null;
  ...
  return (
    <div className="space-y-4">
      ...
      {creating && (
        <Modal title="Novo cargo" onClose={() => setCreating(false)} size="lg">
          <CargoEditor
            pending={pending}
      ...
      {editing && (
        ...
        >
          <CargoEditor
            initialNome={editing.nome}
            initialPerms={editing.permissoes}
            pending={pending}
            onSubmit={(n, p) => handleUpdate(editing.id, n, p)}
