...
export function FormsBoard({
  ...
  const [pastas, setPastas] = useState<Pasta[]>(() => {
    try {
      const s = localStorage.getItem(`folderscache:${membroId}`);
  ...
  const [ordemPasta, setOrdemPasta] = useState<Record<string, string[]>>(() => {
    try {
      const s = localStorage.getItem(`folderforms:${membroId}`);
  ...
  const ordenados = useMemo(() => {
    ...
    if (modo === "za")
      return [...base].sort((a, b) => b.nome.localeCompare(a.nome));
  ...
  async function criarPasta(nome: string, icone: string, formularioIds: string[]) {
    ...
    salvarIcone(String(nova.id), icone);
  ...
  const modoLabel = t(ORDENAR.find((o) => o.v === modo)?.label ?? "Ordenar");
    ...
    <div className="space-y-3">
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder={t("Buscar checklist…")}
          aria-label={t("Buscar checklist")}
      ...
      <div className="flex items-center gap-1.5">
        ...
        >
          <FolderPlus className="h-3.5 w-3.5" /> {t("Criar pasta")}
        </button>
      </div>
      ...
      {semNada ? (
        <div className="flex flex-col items-center justify-center gap-3 py-12 text-center">
          ...
          <p className="font-medium">{t("Nenhum checklist")}</p>
          <p className="max-w-xs text-sm text-muted-foreground">
            {busca || filtroAtivo
              ? t("Ajuste a busca ou os filtros.")
              : t("Quando o gestor liberar checklists para você, eles aparecem aqui.")}
          </p>
        </div>
      ) : (
        ...
        createPortal(
          <>
            ...
            >
              ...
              >
                <Pencil className="h-4 w-4 opacity-70" /> {t("Editar")}
              </button>
              <button
                type="button"
                onClick={() => {
              ...
              >
                <Trash2 className="h-4 w-4" /> {t("Apagar")}
              </button>
            </div>
          </>,
          document.body,
        ...
        >
          <span className="min-w-0 flex-1">
            ...
            <span className="block text-[11.5px] text-muted-foreground">
              {t("{n} checklist(s)", { n: pasta.forms.length })}
            </span>
          </span>
          <ChevronDown
        ...
        <div className="space-y-2 border-t border-border bg-muted/30 p-3">
          {pasta.forms.length === 0 ? (
            <p className="px-1 py-2 text-center text-xs text-muted-foreground">
              {t("Nenhum checklist nesta pasta (ou filtrado pela busca).")}
            </p>
          ) : (
            pasta.forms.map((f) => (
              <FormCard key={f.id} form={f} membroId={membroId} />
...
// Modal de pasta: nome + ícone (sempre) + seleção de checklists (só ao criar).
function PastaModal({
  modo,
  inicialNome,
  inicialIcone,
      ...
      <div className="flex max-h-[85vh] w-full max-w-md flex-col overflow-hidden rounded-t-2xl border border-border bg-card shadow-2xl sm:rounded-2xl">
        <div className="flex items-center gap-3 border-b border-border p-4">
          ...
          <h2 className="flex-1 text-base font-bold">
            {editar ? t("Editar pasta") : t("Criar pasta")}
          </h2>
        </div>
        ...
        {editar && (
          <div className="flex gap-1 border-b border-border p-2">
            {[
              { v: "visual" as const, label: t("Visual") },
              { v: "formularios" as const, label: t("Formulários") },
            ].map((t) => (
              <button
                key={t.v}
                type="button"
            ...
            <>
              <div>
                ...
                <input
                  ...
                  autoFocus
                  placeholder={t("Ex.: Manhã, Frios, Auditorias…")}
              ...
              <div>
                <p className="mb-1.5 text-sm font-medium">{t("Ícone")}</p>
              ...
              {!editar && (
                <div>
                  <p className="mb-1.5 text-sm font-medium">{t("Adicionar checklists")}</p>
                          ...
                          <span className="min-w-0 flex-1">
                            ...
                            {jaEm ? (
                              <span className="block text-xs text-muted-foreground">
                                {t("Já está em outra pasta — será movido")}
                              </span>
                            ) : null}
                          </span>
                        </label>
          ...
          {editar && aba === "formularios" && (
            <div>
              <p className="mb-2 text-xs text-muted-foreground">
                {t("Arraste para mudar a ordem dos checklists dentro da pasta.")}
              </p>
              {ordemForms.length === 0 ? (
                <p className="py-6 text-center text-sm text-muted-foreground">
                  {t("Nenhum checklist nesta pasta.")}
                </p>
              ) : (
                <DndContext
                  sensors={sensors}
...
function FormCard({ form, membroId }: { form: FormItem; membroId: string }) {
  ...
  return (
    <Link
      ...
      className="flex items-center gap-3 rounded-2xl border border-border bg-card p-3.5 shadow-sm transition-all hover:border-primary hover:shadow-md"
    >
      {inner}
    </Link>
  );
...
function SortableCard({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className="flex items-center gap-2 rounded-2xl border border-border bg-card p-3.5 shadow-sm"
    >
      <button
        {...attributes}
        {...listeners}
      ...
      {fora ? (
        <div
          aria-disabled="true"
          className="flex min-w-0 flex-1 cursor-not-allowed items-center gap-3 opacity-70"
        >
          {conteudo}
        </div>
      ) : (
